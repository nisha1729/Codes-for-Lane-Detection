...
class VOCAugDataSet(Dataset):
    def __init__(self, dataset_path='D:/AWSi/LaneNet/Codes-for-Lane-Detection/ERFNet-CULane-PyTorch/list/', data_list='train', transform=None):

        with open(os.path.join(dataset_path + data_list + '.txt')) as f:
            # print("File Opened: ", os.path.join(dataset_path + data_list + '.txt'))
            self.img_list = []
            self.img = []
            self.label_list = []
            self.exist_list = []
...
class VOCAugDataSetPredict(Dataset):
    def __init__(self, dataset_path='D:\AWSi\LaneNet\Codes-for-Lane-Detection\ERFNet-CULane-PyTorch\list',
                 data_list='train', transform=None):
        ...
        with open(os.path.join(dataset_path, data_list + '.txt')) as f:
            ...
            for line in f:
                ...
                self.img_list.append(dataset_path.replace('/list', '') + line.strip().split(" ")[0])
                self.label_list.append(dataset_path.replace('/list', '') + line.strip().split(" ")[1])
                self.exist_list.append(np.array(
                    [int(line.strip().split(" ")[2]), int(line.strip().split(" ")[3]), int(line.strip().split(" ")[4]),
                     int(line.strip().split(" ")[5])]))
